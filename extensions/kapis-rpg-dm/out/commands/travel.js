 ...
 */
async function travel(context, destination) {
    ...
    try {
        ...
        const locationMetadataPath = path.join(workspaceRoot, 'game-data', 'locations', currentLocation, 'metadata.yaml');
        let metadata;
        try {
        ...
        const connectionEntry = connections.find((conn) => (typeof conn === 'string' && conn === destination) ||
            (typeof conn === 'object' && conn.location_id === destination));
        if (!connectionEntry) {
            const validDestinations = connections.map((conn) => typeof conn === 'string' ? conn : conn.location_id).join(', ');
            return {
                success: false,
        ...
        if (triggerResult.success && triggerResult.data.triggeredEvents?.length > 0) {
            ...
            vscode.window.showWarningMessage(`Travel interrupted by event: ${encounter.name}. (Event execution not yet fully implemented)`);
        ...
        if (!visitedList.some((v) => v.locationId === destination)) {
            visitedList.push({
        ...
        const contextResult = await loader.loadContext(sessionState.character.filePath, destination, sessionState, 3500);
        ...
        const hasVisited = sessionState.location.visitedThisSession.filter((loc) => loc === destination).length > 1;
        ...
        return {
            success: false,
            message: `Unexpected error during travel: ${error.message}`,
            error: error
        };
    }
}
