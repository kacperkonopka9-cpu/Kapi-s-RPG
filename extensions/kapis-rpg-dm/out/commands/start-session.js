 ...
 */
async function startSession(context, location, character) {
    ...
    try {
        ...
        const autoSaveInterval = config.get('autoSaveInterval', 300);
        const sessionResult = await sessionManager.startSession(characterPath, // Use absolute path
        location, autoSaveInterval);
        ...
        const contextResult = await loader.loadContext(characterPath, // Use absolute path
        location, sessionState, 3500 // Token budget (3500 soft limit)
        );
        ...
        const templateResult = await templateEngine.renderTemplate('location-initial-visit', gameContext);
        ...
        await sessionManager.updateSession({
            npcs: {
                ...sessionState.npcs,
                activeNPCs: gameContext.npcs?.map((npc) => npc.npcId) || []
            },
            context: {
        ...
        return {
            success: false,
            message: `Unexpected error starting session: ${error.message}`,
            error: error
        };
    }
}
