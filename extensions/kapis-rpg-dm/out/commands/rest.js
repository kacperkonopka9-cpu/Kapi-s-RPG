 ...
 */
async function rest(context, restType) {
    ...
    try {
        ...
        if (restType === 'short') {
            ...
            characterData.hit_dice.current = Math.min((hitDice.current || 0) + hitDiceRecovered, hitDice.max);
        ...
        return {
            success: false,
            message: `Unexpected error during rest: ${error.message}`,
            error: error
        };
    }
}
